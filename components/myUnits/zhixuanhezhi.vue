<template>
	<view>
		...
		<view v-if = "weishu" class="uni-flex uni-row">
			<view  class="flex-item r-top-buttom uni-border" v-for="(item, index) in weishu" :key="index" 
			 @click="clickbuttom1(item,index)" 
		...
		<view v-for="(item,index) in hezhi" :key = "index" class="uni-border">
			...
			<view class="buttom">
					<view v-for="(it, idx) in item.nums" :key="idx" @click="clickbuttom2(it,idx)" class="button-list" :class="in_array(it,click_list2)?'uni-bg-red':'button-light'">{{it}}</view>			
			</view>
		</view>
	...
	.uni-border {
		...
		text-align: center;
			...
			.button-list {
				...
				margin: $uni-xuanma-margin-top-bottom $uni-xuanma-margin-left-right;
				border: 1px $uni-border-color solid;
